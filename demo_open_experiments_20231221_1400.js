...
function create ()
{
    ...
    experiment_item_2.on('pointerup', function (pointer) {
            ...
            experiment_item_2_details.currentStatus = 'moving to workbench';
      },this);
      ...
      experiment_item_3.on('pointerup', function (pointer) {
        ...
        experiment_item_3_details.currentStatus = 'moving to workbench';
...
function update ()
{
    ...
    if (experiment_item_1_details.currentStatus=='move to workbench')
    {
        ...
        if ( stop_if_arrived(experiment_item_1_details, a_bench_target , 'on bench') == true) 
        {
            console.log ("BENCH!!!" , experiment_item_1_details.currentStatus);
        }
    }

    // chekc that we can break the loop if we need to
    if (experiment_item_1_details.currentStatus == 'on bench')
    {
        experiment_item_1_details.currentStatus = 'move to shelf';
        this.physics.moveToObject(experiment_item_1_details.sprite, a_shelf_target, 600);
    }


    if (experiment_item_1_details.currentStatus=='move to shelf')
    {
        if (stop_if_arrived(experiment_item_1_details, a_shelf_target,this, 'on shelf') ) 
        {
            console.log ("SHELF!!!");
...
/*
    ...
    a_target.x = 400;
    a_target.y = 400;
    if (stop_if_arrived(experiment_item_1_details, a_target, this, 'test 1'))
    {
        a_target.x = 100;
        a_target.y = 100;
        console.log ("arrived test 1");
        this.physics.moveToObject(experiment_item_1_details.sprite, a_target, 600)
    };
*/
    //this.physics.moveToObject(experiment_item_1_details.sprite, a_target, 600);
